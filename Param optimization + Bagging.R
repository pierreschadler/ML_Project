...
# --------------  Bagging Begins --------------
#optimization of paramaters  
nvar <- ncol(df)-1
system.time(
  rf.tune <- tune.randomForest(x = train_data[complete.cases(train_data),-1], 
                               y = train_data[complete.cases(train_data),1],
                               nodesize = c(10,50,100), mtry = c(1,4,8,nvar), ntree = c(50,100,200))
)
